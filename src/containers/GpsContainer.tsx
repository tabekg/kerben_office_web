...
export default function GpsContainer() {
  const {t} = useTranslation()

  const [showHiddenItems, setShowHiddenItems] = useState(false)

  const [items, setItems] = useState<IInvoice[]>(
    JSON.parse(localStorage.getItem('__gps_invoices') || '[]')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('__gps_invoices', JSON.stringify(items))
    const items_sync = JSON.parse(
      localStorage.getItem('__gps_invoices_sync') || '[]'
    )
    if (
      items_sync.length < 1 ||
        ...
        <div className='d-flex justify-content-between gap-5 mb-3 align-items-center'>
          <div className='d-flex gap-3'>
            <h1 className='h3 text-muted'>
              GPS квитанции ({renderList.length})
            </h1>
